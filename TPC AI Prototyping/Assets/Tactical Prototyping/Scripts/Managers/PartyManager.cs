...
namespace RTSPrototype
{
    public class PartyManager : MonoBehaviour
    {
        ...
        #region Handlers
        void HandleNoPartyMembers(PartyManager _partyMan, AllyMember _lAlly, bool _onDeath)
        {
            AllyInCommand = null;
            gamemode.HandlePartyMemberWOutAllies(_partyMan, _lAlly, _onDeath);
        }
        void HandleLeftClickPartyMember(AllyMember ally)
        {
        ...
        void HandleRightClick(rtsHitType hitType, RaycastHit hit)
        {
            if (!isCurrentPlayerCommander || noPartyCommandsAllowed) return;
            switch (hitType)
            {
